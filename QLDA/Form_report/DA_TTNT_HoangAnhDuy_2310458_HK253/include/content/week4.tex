...
\tocsubsubsection{4.1 Mô tả công việc}
...
\tocsubsubsection{4.2 Nội dung thực hiện}

\tocsubsubsubsection{4.2.1 Bối cảnh}
...
\tocsubsubsubsection{4.2.2 Khảo sát phần cứng máy lọc nước}
...
\tocsubsubsubsection{4.2.3 Phân tích hệ thống điều khiển}
...
\tocsubsubsubsection{4.2.4 Phân tích vấn đề}
...
\tocsubsubsubsection{4.2.5 Đánh giá các phương án}
...
\tocsubsubsubsection{4.2.6 Lựa chọn giải pháp}
...
Ngoài mô hình ESP32 NodeMCU thông dụng, các dòng ESP32-S2, ESP32-S3, ESP32-C2, ESP32-C3, ESP32-C5 và ESP32-C6 cũng được xem xét như các phương án phần cứng thay thế. Nhóm S2 và S3 phù hợp khi cần bổ sung năng lực xử lý hoặc các giao tiếp ngoại vi; nhóm C2 và C3 phù hợp với các nút thu thập dữ liệu có yêu cầu tiết kiệm chi phí; trong khi C5 và C6 là lựa chọn có thể cân nhắc khi cần khai thác các chuẩn kết nối không dây mới. Việc lựa chọn model cụ thể cần dựa trên số lượng chân GPIO, giao tiếp cần sử dụng, mức tiêu thụ điện năng, khả năng cung ứng linh kiện và yêu cầu mở rộng của hệ thống.
...
\tocsubsubsubsection{4.2.7 Thiết kế giải pháp ESP32}
...
\tocsubsubsubsection{Hardware Interface Design}

\textbf{ESP32 -- TDS Sensor Integration}

Nội dung này mô tả nguyên lý thiết kế giao tiếp phần cứng giữa cảm biến TDS và ESP32 trong phương án sử dụng tín hiệu analog. Thiết kế tập trung vào luồng tín hiệu, các ràng buộc điện và những yếu tố ảnh hưởng đến độ tin cậy; các chi tiết triển khai phần mềm không thuộc phạm vi trình bày.

\textbf{Mô hình kết nối}

\begin{center}
\begin{tabular}{c}
\textbf{TDS Probe} \\
$\downarrow$ \\
\textbf{TDS Signal Conditioning Module} \\
$\downarrow$ \\
\textbf{ESP32 ADC} \\
$\downarrow$ \\
\textbf{Firmware} \\
...
Đầu dò TDS tiếp xúc với nước và tạo ra tín hiệu điện phản ánh đặc tính dẫn điện của mẫu đo. Do tín hiệu từ đầu dò không phù hợp để đưa trực tiếp vào vi điều khiển, TDS Signal Conditioning Module đảm nhiệm kích thích đầu dò, điều hòa và chuyển đổi tín hiệu thành điện áp analog có thể đo được. Bộ chuyển đổi ADC của ESP32 lấy mẫu điện áp này; firmware thực hiện xử lý, chuẩn hóa giá trị TDS và chuyển dữ liệu sang lớp truyền thông MQTT để gửi đến hệ thống giám sát.

\textbf{Electrical Design Considerations}

Thiết kế điện phải được kiểm tra theo thông số của module TDS được lựa chọn. Điện áp hoạt động của module và dải điện áp đầu ra analog cần được xác nhận trước khi kết nối với ADC của ESP32. Tín hiệu đưa vào GPIO hoặc ADC không được vượt quá giới hạn điện áp do nhà sản xuất ESP32 quy định; khi dải đầu ra của module không tương thích, cần sử dụng mạch chia áp, đệm hoặc giải pháp bảo vệ phù hợp.

ESP32 và module TDS phải sử dụng chung điểm tham chiếu GND để bảo đảm phép đo ổn định. Chân ADC được lựa chọn cần phù hợp với kiến trúc phần cứng và không xung đột với các ngoại vi khác. Khối nguồn nên có độ ổn định tốt, được bố trí tụ lọc và đường hồi dòng hợp lý nhằm hạn chế nhiễu trên tín hiệu analog. Đường tín hiệu cần ngắn, tránh đi song song với các đường nguồn hoặc đường chuyển mạch có mức nhiễu cao. Đầu dò TDS không được nối trực tiếp với ESP32 mà bắt buộc phải đi qua mạch xử lý tín hiệu chuyên dụng.

\textbf{Analog Signal Processing}

ESP32 lấy mẫu tín hiệu bằng bộ ADC và firmware chuyển giá trị số thu được thành điện áp tương ứng. Điện áp sau đó được quy đổi thành giá trị TDS dựa trên đặc tính và dữ liệu hiệu chuẩn của module. Để giảm ảnh hưởng của nhiễu tức thời, hệ thống có thể áp dụng bộ lọc Moving Average hoặc Median Filter trước khi sử dụng kết quả đo. Khi giải pháp được bổ sung cảm biến nhiệt độ, dữ liệu nhiệt có thể được dùng để bù ảnh hưởng của nhiệt độ nước đối với phép đo TDS. Phần này chỉ xác định nguyên tắc xử lý, không quy định công thức chuyển đổi cụ thể.

\textbf{Reliability Considerations}

Độ chính xác của phép đo chịu ảnh hưởng bởi nhiễu nguồn, nhiễu điện từ phát sinh trong thời gian ESP32 truyền Wi-Fi, chiều dài và cách bố trí dây tín hiệu analog, sai số của bộ ADC, trạng thái hiệu chuẩn của cảm biến, nhiệt độ nước và chất lượng của module TDS. Vì vậy, đánh giá độ tin cậy cần xem xét đồng thời thiết kế nguồn, bố trí mạch, quy trình lấy mẫu và điều kiện vận hành thực tế. Việc hiệu chuẩn định kỳ bằng dung dịch tham chiếu phù hợp là cơ sở để duy trì tính nhất quán của dữ liệu đo.

\textbf{Design Decisions}

\begin{itemize}
    \item Sử dụng module TDS có mạch xử lý tín hiệu thay vì đọc trực tiếp tín hiệu từ đầu dò.
    \item Thực hiện lọc dữ liệu tại firmware trước khi phát bản tin MQTT nhằm hạn chế nhiễu và giá trị bất thường.
    \item Chuẩn hóa dữ liệu TDS trước khi gửi đến Device Monitor để bảo đảm tính nhất quán trong quá trình tiếp nhận, lưu trữ và hiển thị.
    \item Duy trì khả năng mở rộng để bổ sung chức năng hiệu chuẩn và bù nhiệt độ trong các phiên bản tiếp theo.
\end{itemize}

\vspace{0.5em}

\tocsubsubsubsection{4.2.8 Tích hợp HiveMQ}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.9\textwidth]{include/image/ESP32_NodeMCU.jpg}
...
\tocsubsubsection{4.3 Kết quả đạt được}
