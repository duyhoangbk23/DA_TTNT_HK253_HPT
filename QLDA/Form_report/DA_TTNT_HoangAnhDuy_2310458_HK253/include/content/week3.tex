...
\paragraph{
SmartWater Admin được triển khai dưới dạng ứng dụng Laravel nguyên khối phân lớp. Một yêu cầu đi qua chuỗi xử lý \textit{Route $\rightarrow$ Controller $\rightarrow$ Service $\rightarrow$ Eloquent Model $\rightarrow$ MySQL}, sau đó kết quả được trình bày bằng Blade View.

Các chức năng nghiệp vụ được tổ chức theo từng nhóm trong cùng một ứng dụng. Cách phân lớp giúp tách giao diện, điều phối yêu cầu, xử lý nghiệp vụ và truy cập dữ liệu, nhưng SmartWater Admin vẫn là một khối triển khai thống nhất chứ không phải hệ thống microservice.
...
Toàn bộ telemetry được Device Monitor xử lý trước khi lưu vào MySQL \texttt{smartwater\_database}. SmartWater Admin sử dụng cùng nguồn dữ liệu và mô hình quan hệ \textit{Customer $\rightarrow$ Contract $\rightarrow$ Product $\rightarrow$ Device + MCU $\rightarrow$ Telemetry} để phục vụ giám sát và quản lý.
...
\begin{enumerate}
    ...
    \item Thông qua trường \texttt{mcu\_id}, hệ thống liên kết telemetry với MCU. MCU và Device cùng thuộc một Product; Product thuộc tối đa một Contract và Contract thuộc một Customer.
...
\subsubsection*{3.5 Thiết kế cơ sở dữ liệu}
\paragraph{
Cơ sở dữ liệu dùng chung được thiết kế theo mô hình quan hệ để kết nối dữ liệu
nghiệp vụ và dữ liệu giám sát IoT. Các migration trong dự án
\texttt{smartwater-database} là nguồn chuẩn của schema; SmartWater Admin và
Device Monitor cùng sử dụng database \texttt{smartwater\_database}.
}

Các thực thể nghiệp vụ được liên kết theo chuỗi \textit{Customer $\rightarrow$
Contract $\rightarrow$ Product $\rightarrow$ Device + MCU}. Bảng
\texttt{telemetry} lưu lịch sử TDS theo \texttt{mcu\_id}; trường này là chuỗi
tối đa 50 ký tự, có index riêng và index kết hợp với \texttt{timestamp} để hỗ
trợ truy vấn telemetry theo thời gian. Ngoài ra, các nhóm bảng người dùng và
phân quyền, danh mục sản phẩm, kho/lô hàng, bảo trì, nhật ký hoạt động và tệp
đính kèm phục vụ các nghiệp vụ quản trị của SmartWater Admin.

Hình \ref{fig:db-design} mô tả các bảng dữ liệu và quan hệ chính của hệ thống.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\linewidth]{include/image/DB_Design.png}
    \caption{Thiết kế cơ sở dữ liệu SmartWater}
    \label{fig:db-design}
\end{figure}

\subsubsection*{3.6 Lựa chọn công nghệ}
\paragraph{
...
\subsubsubsection*{3.6.1 MQTT và HiveMQ}
\paragraph{
...
\subsubsubsection*{3.6.3 Laravel}
\paragraph{
...
\subsubsubsection*{3.6.4 ASP.NET Core}
\paragraph{
