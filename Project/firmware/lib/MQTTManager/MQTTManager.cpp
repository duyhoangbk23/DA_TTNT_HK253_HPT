...
bool MQTTManager::publishTelemetry(const TelemetryData& telemetry) {
    if (!_client.connected()) {
        return false;
    }
    ...
    document["temperature"] = telemetry.temperature;
    document["tds"] = telemetry.tds;
    document["pressure"] = telemetry.pressure;
    document["flow"] = telemetry.flow;
    document["turbidity"] = telemetry.turbidity;
    document["water_level"] = telemetry.waterLevel;
    document["leak"] = telemetry.leak;
    document["wifi_rssi"] = telemetry.wifiRssi;

    return publishDocument(Config::Topics::TELEMETRY, document, false);
}

bool MQTTManager::publishStatus(const TelemetryData& telemetry, const char* status) {
    if (!_client.connected()) {
        return false;
    }
